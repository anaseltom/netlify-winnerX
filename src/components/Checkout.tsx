...
const Checkout: React.FC<any> = ({feature, title, filterControl}) => {
    ...
    const getProd = (code:any) => {
        var prodIndex = products_list?.findIndex((s: any) => s.product_code === code);
                    ...
                    <div className="row">
                        <div className="col-lg-12">
                            <div className="shoping__cart__btns">
                                <Link to={`/home`}><button className="primary-btn cart-btn">CONTINUE SHOPPING</button></Link>
                        ...
                        <div className="col-lg-6">
                            <div className="shoping__checkout">
                                <h5>Cart Total</h5>
                                <ul>
                                    <li>Subtotal <span>{numberWithCommas(products_cart_total?.total)}</span></li>
                                    <li>Total <span>{numberWithCommas(products_cart_total?.total)}</span></li>
                                </ul>
                                {
                                    products_cart.length > 0 ?
                                    <Link to={`/confirm_checkout`}><button className="primary-btn checkout_button">PROCEED TO CHECKOUT</button></Link>
                                    :
                                    <Link onClick={ (event) => event.preventDefault() } to={`/confirm_checkout`}><button className="warning-btn checkout_button">PROCEED TO CHECKOUT</button></Link>
                                }
                            </div>
                        </div>
                    </div>
