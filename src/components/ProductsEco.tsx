    ...
    return (
        <section className="featured spad prod_eco">
            <div className="" style={{overflowX: "hidden", position: "relative"}}>
                ...
                <div className="row featured__filter" style={{justifyContent: "flex-start", backgroundImage: `url('assets/img/slide-02.jpg')`, overflowX: "auto", flexWrap: "nowrap"}}>
                    {
                    products_list && products_list.length >0 && products_list.filter((product:any)=>filterProd(product)).slice(0, 10).map((ar: any, ctr: number) => {
                        return (

                            <div onClick={(e)=> {setProdIsOpen(true);setUpdateProdSummary(ar);}} key={ar.id} className={`col-lg-2 col-md-3 col-sm-4 mix ${truncateString(ar.category_name, 15)}`}>
                                <div className="featured__item">

                                    <div className="prod_qty_info">
                                        <span className="info_text">{} sold out of 100</span>
                                        <div className="bar"><div className="progress"></div></div>
                                    </div>

                                    <div className="items_wrapper">
                                        <div className="image_wrapper" style={{backgroundImage: `url('${ar?.image_url_main}')`}}>
                                            {/* <img src={ar?.image_url_main} alt=""/> */}
                                        </div>
                                        <div className="item_title">{truncateString(ar?.product_name, 15)}</div>
                                    </div>
