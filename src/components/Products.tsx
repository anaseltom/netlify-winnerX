    ...
    return (
        <section className="featured spad prod_cat">
            <div className="container">
                    ...
                    <div className="col-lg-12">
                        ...
                        </div>
                        {
                            ...
                            <div className="featured__controls">
                                ...
                                {
                                    feature=="" &&
                                    <li className={activeTab==1?"active" : ""} onClick={ () => {setFilterProduct(""); setActiveTab(1)} } data-filter="*">All</li>
                                }
                                { 
                                    feature=="" && categories && categories.length >0 && categories.map((ar: any, i:number) => {
                                        i+=1;
                                        return (
                                        <li className={activeTab==(++i)?"active" : ""} key={i} onClick={ () => {setFilterProduct(ar.categoryCode); setActiveTab(i); }} data-filter={ `.${ar.categoryName}` }>{ar.categoryName}</li>
                ...
                <div className="row featured__filter" style={{justifyContent: "center"}}>
                    {
                    products_list && products_list.length >0 && products_list.filter((product:any)=>filterProd(product)).map((ar: any, index: number) => {
                        return (

                            <div onClick={(e)=> {setProdIsOpen(true);setUpdateProdSummary(ar);}} key={index} className={`col-lg-2 col-md-3 col-sm-6 mix ${ar.categoryName}`}>
                                <div className="featured__item">
                                    <div className="featured__item__pic set-bg" style={{backgroundImage: `url('${ar.image_url_main}')`, backgroundSize: "contain" }} data-setbg="/assets/img/featured/feature-1.jpg">
